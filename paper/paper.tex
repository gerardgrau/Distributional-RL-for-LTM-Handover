...
Our contributions are:\begin{itemize}
  ...
  output size and inference cost of scalar DQN (Section~\ref{sec:results-bridge}).
...
single quantile at $\tau=0.5$ closely reproduces DQN across every metric
...
few and diminishing returns afterward: sweeping $N$ at the lighter study setting
($1{,}000$ episodes, three seeds), the
...
while the reward per decision stays flat (${\approx}3.66$). We therefore carry the
$N{=}25$ head throughout; at the final $2{,}000$-episode, five-seed setting it
remains the best-performing mean policy (Table~\ref{tab:dist}, right column), and
it supplies the finer tail grid that the CVaR and
...
best value in each row (ties at the shown precision broken by the unrounded value).}
\label{tab:alpha}
\centering
\begin{tabular}{|l|c|c|c|}
\hline
...
you act on''---is both the cheapest and the most failure-averse design.
...
markedly higher (HOF $0.42$--$0.48$ versus RA-1q's $0.374$ in the $1{,}000$-episode studies), while
...
LTM baseline leads is resource reservation: the learned agents hold more cells
...
$28$/min, lowest for CMAB at $24.1$ and RA-1q at $24.7$).
...
\textbf{LTM}: our hand-tuned baseline. \textbf{CMAB}$^{*}$: the contextual
multi-armed bandit of~\cite{cmab_ho}. \textbf{DQN}: scalar baseline. \textbf{RN}: risk-neutral QR-DQN
...
quality KPIs; the only metric on which the LTM baseline leads is resource
