...
\author{\IEEEauthorblockN{Gerard Grau Garcia %,Marga Cabrera-Bean, Josep Vidal, Ainna Yue Moreno-Locubiche%
}
\IEEEauthorblockA{\textit{\textsuperscript{\Star}Dept. of Signal Theory and Communications, Universitat Politècnica de Catalunya - BarcelonaTech (UPC), Spain}   \\
{gerard.grau.garcia@estudiantat.upc.edu}}
%\and
...
Yet in LTM the user experience is dominated not by the average step but by rare, disproportionately costly tail events---handover and radio-link failures---which a mean-optimal policy has no explicit incentive to avoid.
...
In this paper we approach LTM HO with \emph{distributional} reinforcement learning, which models the full return distribution $Z(x,a)$---for a given state $x$ and action $a$---rather than its mean $Q(x,a)$, and with \emph{risk-aware} action selection based on the Conditional Value at Risk (CVaR) of that distribution.
...
link cannot be established, and a \emph{radio-link failure} (RLF) when the serving link collapses after a sustained out-of-sync observations within a time window.
...
(for example, for a uniform return on $[0,1]$, $\mathrm{CVaR}_{0.2}$ is the mean
of the values in $[0,0.2]$, i.e.\ $0.1$).
...
$\theta_i(x,a)\approx F^{-1}_{Z(x,a)}(\tau_i)$. Vanilla QR-DQN places these at
...
and the per-transition loss sums the pairwise temporal-difference errors between
each of the $N$ predicted quantiles and each of the $N$ target quantiles
$y_j = r + \gamma\,\theta_j\!\big(x',a^\star\big)$,
...
cross-seed mean $\pm$ standard deviation throughout.
...
\end{enumerate}
These are benchmarked against two references: our own hand-tuned LTM baseline,
run in the same simulator, and the contextual-bandit (CMAB) policy
of~\cite{cmab_ho}. We additionally train full- and truncated-CVaR agents,
...
three QR-DQN agents. DQN is a scalar value learner; the per-agent risk rule is
listed with the agents above.}
\label{tab:hparams}
\centering
\begin{tabular}{|l|c|}
\hline
...
set of $1{,}000$ trajectories for $2{,}000$ episodes and evaluated as the average
over five seeds. The two differ only in the output layer: DQN regresses a single
scalar value, whereas RN predicts a distributional \emph{head}---the set of
outputs of the network's final layer, here $25$ quantiles---and acts on its mean.
...
DQN and RN earn the same reward per decision, yet RN is more reliable on
...
unchanged (Table~\ref{tab:dist}). Learning the return distribution and acting on
...
appears only as further quantiles populate the head, and it is largely complete
once a few are present. Their \emph{placement}, by contrast, is immaterial:
...
\begin{table}[htbp]
\caption{Distributional learning ($2{,}000$ episodes, five seeds): the
single-quantile head reproduces DQN, while the full $25$-quantile head lowers the
...
\hline
\textbf{KPI} & \textbf{DQN} & \textbf{RN ($k{=}1$)} & \textbf{RN ($N{=}25$)} \\
\hline
...
distribution. We sweep it over $1{,}000$ episodes and three seeds, and defer how
the tail estimate should be \emph{built} to Section~\ref{sec:results-bridge}.
...
reduction levels off, leaving a clear knee near $\alpha=0.25$, which we adopt.
...
(Fig.~\ref{fig:frontier}). Soft-step is its continuous member: at ratio $r=1$ it is
...
$3.606$ at $r=100$, toward the hard-CVaR value of $3.598$. The single dedicated
...
\begin{table}[htbp]
\caption{The CVaR risk dial ($1{,}000$ episodes, three seeds), from mild
...
Section~\ref{sec:results-dist} showed that representing the \emph{mean} rewards
more quantiles, up to saturation. The risk estimate behaves oppositely: the lower
...
headline single-quantile agent is also reported at the canonical
$2{,}000$-episode, five-seed setting.

\textbf{Quantile count for the tail.} Holding the risk level at $\alpha=0.25$ and
averaging the lowest $k$ quantiles into the CVaR estimate, the reward per decision
...
the canonical setting it attains the lowest failure rates of any learned agent
...
\subsection{Comparison with the LTM Heuristic and Contextual Bandit}
\label{sec:results-main}
Table~\ref{tab:final} reports the four learned agents at the canonical
...
contextual multi-armed bandits (CMAB) of~\cite{cmab_ho}, whose published figures
...
\begin{table*}[t]
\caption{Final performance on the LTM handover task. Learned agents: mean $\pm$ std
on reward per decision over $2{,}000$ episodes and five seeds. \textbf{LTM}: our
hand-tuned baseline, same simulator. \textbf{CMAB}$^{*}$: the contextual bandits
...
Table~\ref{tab:final}). References first---our LTM baseline (blue, our simulator)
...
better when lower. Reward per decision is our training objective, with the LTM
reward measured in our simulator and CMAB reporting none.}
\label{fig:bars}
\end{figure}
...
calibrated in~\cite{cmab_ho} for the CMAB's low-signaling regime rather than
...
Thanks to my tutors Josep Vidal Manzano and Marga Cabrera Bean, for the mentoring and guidance on this paper.
Also many thanks to Ainna Yue Moreno, for providing the code necessary to perform the simulation on the LTM, and to help me debug the simulation.
