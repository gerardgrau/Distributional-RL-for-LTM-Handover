...
class MobilityDashboard:
    ...
    def render_episode(self, history: dict[str, list[Any]], save_path: Optional[str] = None):
        ...
        # Only label Base Stations (not every sector) to keep clean
        ...
        rsrp_lines = []
        # Only plot top few sectors to avoid clutter
        for i in range(min(6, self.num_bs)):
            line, = ax_rsrp.plot([], [], label=f"Sector {i}")
        ...
        def animate(t):
            ...
            ax_rsrp.set_xlim(max(0, t - 200), t + 10)
            
            return [ue_dot, conn_line, traj_line] + rsrp_lines

        # Limit animation frames for presentation GIF size
        total_frames = len(ue_positions)
        # Downsample to ~250 frames for reasonable file size
...
def get_mock_bs_positions(n_bs=7):
    """
    Returns 21 sector positions (3 per BS).
    """
    bs_pos = []
    # 7 BS in a hexagonal layout
    for i in range(n_bs):
        angle = 2 * np.pi * i / (n_bs-1) if i > 0 else 0
        r = 300 if i > 0 else 0
        bx, by = r * np.cos(angle), r * np.sin(angle)
        
        # 3 sectors slightly offset from BS center
        for s in range(3):
            s_angle = 2 * np.pi * s / 3
            offset = 15
